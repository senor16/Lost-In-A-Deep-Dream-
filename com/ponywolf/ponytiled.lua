...
local function centerAnchor(image, anchorX, anchorY)
  anchorX,
    anchorY = anchorX or 0.5, anchorY or 0.5
...
local function decodeTiledColor(hex)
  ...
  local a,
    r,
    g,
    b = hexToFloat(hex:sub(1, 2)), hexToFloat(hex:sub(3, 4)), hexToFloat(hex:sub(5, 6)), hexToFloat(hex:sub(7, 8))
  return r, g, b, a
end
...
local function snap(map, object, alignment, margin, safe)
  ...
  local anchorX,
    anchorY = object.anchorX, object.anchorY
  ...
  object.x,
    object.y = map:contentToLocal(x, y)
...
function M.new(data, dir)
  ...
  local width,
    height = data.width * data.tilewidth, data.height * data.tileheight
  local sheets = {}
  ...
  local function loadTileset(num)
    ...
    local tsiw,
      tsih = tileset.imagewidth, tileset.imageheight
    local margin,
      spacing = tileset.margin or 0, tileset.spacing or 0
    local w,
      h = tileset.tilewidth, tileset.tileheight
      ...
      for ty = 0, data.height - 1 do
        for tx = 0, data.width - 1 do
          ...
          local gid,
            flip,
            sheet,
            animation = gidLookup(tileNumber)
          if gid then
            local image
            if animation then
              --print("Animating:", gid)
            ...
            image.x,
              image.y = tx * data.tilewidth, (ty + 1) * data.tileheight
            centerAnchor(image)
      ...
      for j = 1, #layer.objects do
        ...
        if object.gid then
          local gid,
            flip,
            sheet,
            animation = gidLookup(object.gid)
          if gid then
            local image
            if animation then
              --print("Animating:", gid)
            ...
            image.type = object.class
            image.id = object.id
            image.filename = sheet and "none" or (dir .. gid)
            -- apply base properties
            local anchorX,
              anchorY = object.properties.anchorX, object.properties.anchorY
            object.properties.anchorX,
              object.properties.anchorY = nil, nil
            image.anchorX,
              image.anchorY = 0, 1
            image.x,
              image.y = object.x, object.y
        ...
        elseif object.polygon or object.polyline then -- Polygon/line
          ...
          if object.polygon then
            local xMax,
              xMin,
              yMax,
              yMin = -4294967296, 4294967296, -4294967296, 4294967296 -- 32 ^ 2 a large number
            for p = 1, #points do
            ...
            originX,
              originY = (xMax + xMin) / 2, (yMax + yMin) / 2
            ...
            originX,
              originY = points[1].x, points[1].y
            for p = 3, #points do
              polygon:append(points[p].x, points[p].y)
            end
            polygon.x,
              polygon.y = object.x, object.y
            polygon:translate(originX, originY)
          end
  ...
  function map:centerObject(obj, tween)
    ...
    local objx,
      objy = obj:localToContent(0, 0)
    objx,
      objy = centerX - objx, centerY - objy
    if tween then
      self.x,
        self.y = self.x + objx / 8, self.y + objy / 8
    else
      self.x,
        self.y = self.x + objx, self.y + objy
    end
  end
  ...
  function map:boundsCheck(border)
    if self.translate then
      border = border or 0
      local xMax,
        yMax = width * self.xScale, height * self.yScale
  ...
  map.designedWidth,
    map.designedHeight = width, height
